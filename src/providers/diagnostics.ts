...
import { canonicalCasing, knownKeywordsLower } from '../options'
import { lintText } from '../lint-core'
import { isSupportedLanguage } from './utils'

const COLLECTION_NAME = 'ssh-config-pro'

/**
 * Lints SSH config documents, flagging directives that are not recognised SSH
 * keywords. Behaviour is controlled by the `sshConfigPro.lint.*` settings.
 */
export class SSHDiagnosticsProvider {
  ...
  constructor(context: ExtensionContext) {
    const subs: Disposable[] = [
      ...
      workspace.onDidChangeConfiguration((e) => {
        if (e.affectsConfiguration('sshConfigPro.lint')) {
          this.lintAll()
        }
      }),
    ]
  ...
  private lint(document: TextDocument): void {
    ...
    const config = workspace.getConfiguration('sshConfigPro.lint')
    ...
    const diagnostics: Diagnostic[] = lintText(document.getText(), known, canonical).map((f) => {
      ...
      return { range, message, severity, source: 'ssh-config-pro' }
    })
